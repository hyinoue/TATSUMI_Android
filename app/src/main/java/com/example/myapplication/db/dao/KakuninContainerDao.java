...
public interface KakuninContainerDao {
    ...
    @Query("SELECT * FROM T_KAKUNIN_CONTAINER WHERE CONTAINER_ID = :containerId")
    KakuninContainerEntity findByContainerId(String containerId);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void upsert(KakuninContainerEntity entity);

    @Query("SELECT * FROM T_KAKUNIN_CONTAINER " +
            "WHERE CONTAINER_SYOUGO_KANRYO = 1 AND DATA_SEND_YMDHMS IS NULL " +
            "ORDER BY CONTAINER_ID")
    List<KakuninContainerEntity> findUnsentCompleted();

    @Query("UPDATE T_KAKUNIN_CONTAINER SET DATA_SEND_YMDHMS = :dataSendYmdhms WHERE CONTAINER_ID = :containerId")
